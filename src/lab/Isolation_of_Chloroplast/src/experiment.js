...
var hit_image_x, hit_image_y, hit_max_x, hit_max_y;
 ...
 return {
  ...
  link: function(scope, element, attrs) {
   ...
   queue.loadManifest([{
    ...
    id: "tubestandunder",
    src: "././images/tube_stand_under.svg",
    type: createjs.LoadQueue.IMAGE
   }, {
    id: "tube_one",
   ...
   function handleComplete() {
    ...
    createCircle("standard_circle_button", "standard_circle", 424, 299, system_container, 34, "#0947A3", "#000000");
    createCircle("blank_circle_button", "blank_circle", 489, 339, system_container, 34, "#D6D6D6", "#0947A3");
    createCircle("sample_circle_button", "sample_circle", 565, 355, system_container, 34, "#0947A3", "#000000");
    ...
    createRoundRect("round_rect_button_1", "round_rect_1", 120, 320, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_2", "round_rect_2", 201, 320, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_3", "round_rect_3", 280, 320, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_4", "round_rect_4", 119, 360, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_5", "round_rect_5", 200, 360, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_6", "round_rect_6", 279, 360, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_7", "round_rect_7", 118, 400, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_8", "round_rect_8", 200, 400, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_9", "round_rect_9", 278, 400, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_10", "round_rect_10", 117, 440, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_11", "round_rect_11", 200, 440, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_12", "round_rect_12", 277, 440, 70, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_13", "round_rect_13", 473, 441, 39, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_14", "round_rect_14", 473, 400, 39, 37, 25, "#0947A3", "#000000", 0.01, system_container);
    createRoundRect("round_rect_button_15", "round_rect_15", 531, 399, 75, 37, 25, "#0947A3", "#D4D4D4", 0.01, system_container);
    createRoundRect("round_rect_button_15", "round_rect_16", 531, 441, 75, 37, 25, "#0947A3", "#D4D4D4", 0.01, system_container);
    ...
    for (var i = 1; i < numbe_of_round_rect; i++) {
     var _round_rect_button_name = "round_rect_" + i
     round_rect_button_name_to_container = system_container.getChildByName(_round_rect_button_name);
     /**Mouse down function of button. */
     round_rect_button_name_to_container.on("mousedown", function(evt) {
      buttonClick("button_up_" + this.name, "button_text_" + this.name); /**Calling the buttonClick function and send the parameter. - name of the button */
     });
     /**Mouse up function of button. */
     round_rect_button_name_to_container.on("pressup", function(evt) {
      buttonClickUp("button_up_" + this.name, "button_text_" + this.name); /**Calling the buttonClick_up function and send the parameter. - name of the button */
     });
    }
    /**System ON function */
    system_container.getChildByName("round_rect_10").on("pressup", function(evt) {
     systemOn(); /** This function is used to show the arrows and numbers inside the screen. */
    });
    /** Up arrow functionality*/
    system_container.getChildByName("round_rect_14").on("pressup", function(evt) {
     arrowMove(system_container.getChildByName("round_rect_13"), "up", system_container.getChildByName("round_rect_14"));
    });
    /**Down arrow functionality*/
    system_container.getChildByName("round_rect_13").on("pressup", function(evt) {
     arrowMove(system_container.getChildByName("round_rect_14"), "down", system_container.getChildByName("round_rect_13"));
    });
    ...
    system_container.getChildByName("round_rect_16").on("pressup", function(evt) {
     ...
     showHideValues(0, 0); /**Hiding all values inside the screen.*/
    ...
    system_container.getChildByName("switch_to_move").on("pressup", function(evt) {
     if (cap_move_flag) { /**Checking the cap is removed, using cap_move_flag */
      createjs.Tween.get(system_container.getChildByName("switch_to_move"), {
       override: true
      }).to({
       y: 100
      }, 300);
      ...
      cap_move_flag = false;
      for (var i = 1; i < 4; i++) {
       var _test_tube_name = "tube_" + i;
       test_tube_name_container = system_container.getChildByName(_test_tube_name);
       test_tube_name_container.on("pressmove", function(evt) {
        current_test_tube_name = this.name;
        dragAndDropTesttube(current_test_tube_name);
       });
      }
     } else {
      createjs.Tween.get(system_container.getChildByName("switch_to_move"), {
    ...
    function dragAndDropTesttube(current_test_tube_name) {
     ...
     system_container.getChildByName(current_test_tube_name).on("pressup", function(evt) {
      if (tub_current_position_x > hit_image_x && tub_current_position_y < hit_image_y && tub_current_position_x < hit_max_x && tub_current_position_y > hit_max_y && drop_flag == 0) { //Drop area of the test tube.
       drop_flag = 1;
       ...
       revertTubs(); /**Calling the revert function when the tube drop away from the top of the device. it will move towards the original position.  */
      }
     });
    }
   ...
   function buttonClick(element_name, text_Name) {
   ...
   function buttonClickUp(element_name, text_Name) {
    system_container.getChildByName(element_name).alpha = 1;
    system_container.getChildByName(text_Name).y--;
   }
   /**Function is used to create circles on the top of the button */
   function createCircle(circle, circleName, xPos, yPos, container, circle_radius, circle_color, border_color) {
   ...
   function createRoundRect(round_rect, round_rect_name, xPos, Ypos, round_rect_width, round_rect_height, round_rect_radius, border_color, filled_color, alpha_Value, container) {
   ...
   /**when calling the systemOn function, arrow and readings get displayed on the device screen. */
   function systemOn() {
   ...
   function arrowMove(opposite_arrow, position_to_move, current_arrow) {
    if (system_on_off_falg) {
     ...
     if (position_to_move == "up") { /** checking the button is "Arrow-up"  */
      ...
      if (arrow_click_count == 0) {
       showHideValues(1, 0); /**Showing the first section values on the screen */
      ...
      if (arrow_click_count == 4) {
       showHideValues(0, 1); /**Showing the second section values on the screen */
   ...
   function showHideValues(first_section, second_section) {
